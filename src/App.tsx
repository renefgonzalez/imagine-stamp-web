...
// Note: React needs to be imported for the ErrorBoundary to work as a class component
...
export default function App() {
  ...
  const categories = useMemo(() => {
    ...
    // Reconstruct icons for saved categories
    const dynamicCategories = parsed.map((cat: any) => ({
      ...cat,
      icon: ICON_MAP[cat.id] || Sparkles // Default icon if not mapped
    }));

    // Ensure "VER TODO" is always first
    return [DEFAULT_CATEGORIES[0], ...dynamicCategories];
  }, [view]);

  if (view === 'admin') return <AdminPanel onLogout={() => setView('shop')} />;

  const filteredProducts = useMemo(() => {
    // Combinar productos estáticos con los de localStorage
  ...
  console.log('App cargada en vista:', view);
      ...
      <div className="min-h-screen flex flex-col bg-background">
            ...
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="h-14 w-14 flex items-center justify-center overflow-hidden">
                  <img 
                    src={logo} 
                    alt="Imagine & Stamp Logo" 
                    className="h-14 w-auto object-contain [image-rendering:crisp-edges]"
                    onError={(e) => {
                      (e.target as HTMLImageElement).style.display = 'none';
                      (e.target as HTMLImageElement).parentElement?.classList.add('bg-primary', 'rounded-full');
                    }}
                  />
                </div>
        ...
        <main className="pt-16 pb-12 max-w-7xl mx-auto w-full">
          ...
          <section className="relative w-full h-[350px] md:h-[400px] overflow-hidden mb-8">
            <img 
              alt="Imagine & Stamp Creative Studio" 
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-12">
              <motion.h2 
                ...
                className="text-white text-3xl md:text-5xl font-headline font-extrabold leading-tight max-w-2xl drop-shadow-[0_2px_10px_rgba(0,0,0,0.6)]"
              >
          ...
          <div className="px-4">
            {/* Search Bar */}
            <div className="mb-10">
              <div className="relative flex items-center bg-white rounded-full px-5 py-3 shadow-sm border border-primary/10 transition-all focus-within:ring-2 focus-within:ring-primary/20">
                <Search className="text-primary/40 mr-3" size={20} />
                <input 
                  className="bg-transparent border-none focus:outline-none w-full text-sm font-label text-on-surface placeholder:text-primary/30" 
                  placeholder="Busca productos artísticos..." 
                  type="text" 
                />
              </div>
            </div>
            ...
            <section id="catalog" className="mb-16">
              ...
              <div className="flex items-start gap-10 overflow-x-auto hide-scrollbar pb-6 px-4">
                {categories.map((cat) => (
                  ...
                  >
                    <div className={`w-20 h-20 rounded-full flex items-center justify-center transition-all duration-500 ${cat.bgColor} ${selectedCategory === cat.id ? 'ring-4 ring-secondary ring-offset-4 scale-110 shadow-xl' : 'hover:scale-105 shadow-sm'}`}>
                      <cat.icon className={`w-8 h-8 ${selectedCategory === cat.id ? 'text-secondary' : cat.iconColor}`} />
                    </div>
                    <span className={`text-[11px] font-black font-headline uppercase tracking-widest transition-colors text-center max-w-[100px] leading-tight ${selectedCategory === cat.id ? 'text-primary' : 'text-primary/40'}`}>
                      {cat.name}
                    </span>
                  </div>
                ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                      {categories.find(c => c.id === selectedCategory)?.submenus.map((sub, idx) => (
                        <div key={idx} className="flex items-center gap-3 group cursor-pointer">
                          <div className={`w-2 h-2 rounded-full shrink-0 ${categories.find(c => c.id === selectedCategory)?.dotColor} group-hover:scale-150 transition-transform`} />
              ...
              <AnimatePresence mode="popLayout">
                {filteredProducts.map((product) => (
                  <motion.div 
                    ...
                    whileHover={{ y: -8 }}
                    className="masonry-item group bg-white rounded-2xl p-4 shadow-[0_15px_35px_-12px_rgba(0,0,0,0.08)] hover:shadow-[0_25px_50px_-12px_rgba(0,35,102,0.12)] transition-all duration-500"
                  >
                    <div className={`relative aspect-[3/4] rounded-2xl overflow-hidden mb-4`}>
                      <img 
                        ...
                        referrerPolicy="no-referrer"
                      />
                      <button className="absolute top-3 right-3 w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center text-accent-magenta shadow-sm hover:scale-110 transition-transform">
                        <Heart size={20} fill="currentColor" />
                      </button>
                      {product.subCategory && (
                        <span className="absolute bottom-3 left-3 px-2 py-1 bg-primary/80 backdrop-blur-sm text-white text-[10px] font-bold rounded-md uppercase tracking-tighter">
                          {product.subCategory}
                        </span>
                      )}
                    </div>
                    <h3 className="font-headline font-bold text-primary text-sm leading-tight mb-1">{product.name}</h3>
                    ...
                    <div className="flex flex-col gap-2 mt-auto">
                      ...
                      <button 
                        ...
                        className="w-full py-2 bg-secondary text-white rounded-xl font-bold text-xs shadow-md shadow-secondary/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                      >
                        <Plus size={14} />
                        AGREGAR AL CARRITO
                      </button>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <div className="h-[50px] flex items-center justify-center overflow-hidden">
                  <img 
                    src={logo} 
                    alt="Imagine & Stamp Logo" 
                    className="h-[50px] w-auto object-contain [image-rendering:crisp-edges]"
                    onError={(e) => {
                      (e.target as HTMLImageElement).style.display = 'none';
                    }}
                  />
              ...
              <p className="text-primary/60 text-sm leading-relaxed max-w-xs">
                Personalizamos tus momentos más especiales con diseños creativos y acabados de alta calidad.
              </p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <a href="https://wa.me/525650469993" target="_blank" rel="noreferrer" className="flex items-center gap-3 text-primary/70 hover:text-secondary transition-colors group">
                  <div className="w-10 h-10 rounded-full bg-primary/5 flex items-center justify-center group-hover:bg-secondary/10 transition-colors">
                    <MessageCircle size={18} />
                  </div>
                  <span className="text-sm font-medium">WhatsApp</span>
                </a>
                <a href="mailto:imagineandstamp@gmail.com" className="flex items-center gap-3 text-primary/70 hover:text-secondary transition-colors group">
                  <div className="w-10 h-10 rounded-full bg-primary/5 flex items-center justify-center group-hover:bg-secondary/10 transition-colors">
                    <Mail size={18} />
                  </div>
                  <span className="text-sm font-medium">imagineandstamp@gmail.com</span>
                </a>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4">
                <a href="https://www.instagram.com/personalizadosimagineandstamp" target="_blank" rel="noreferrer" className="w-12 h-12 rounded-full bg-primary/5 flex items-center justify-center text-primary/70 hover:text-secondary hover:bg-secondary/10 transition-all">
                  <Instagram size={22} />
                </a>
                <a href="https://www.facebook.com/share/1CFhhieFeV/?mibextid=wwXIfr" target="_blank" rel="noreferrer" className="w-12 h-12 rounded-full bg-primary/5 flex items-center justify-center text-primary/70 hover:text-secondary hover:bg-secondary/10 transition-all">
                  <Facebook size={22} />
                </a>
              </div>
            </div>
          </div>
          <div className="max-w-7xl mx-auto mt-12 pt-8 border-t border-primary/5 text-center relative pb-10">
            <p className="text-primary/30 text-xs font-medium uppercase tracking-widest">
              © 2026 Imagine & Stamp. Todos los derechos reservados.
            </p>
            <button 
              onClick={() => setView('admin')}
              className="mt-4 inline-flex items-center gap-1 text-primary/20 hover:text-primary/60 transition-colors text-[10px] uppercase tracking-tighter"
            >
              <Settings size={14} />
        ...
        <div className="fixed bottom-8 right-6 z-50 flex flex-col gap-4 items-end">
          <motion.a 
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center gap-3 bg-[#25D366] text-white px-6 py-4 rounded-full shadow-2xl shadow-green-500/40 transition-transform group"
          >
            <span className="font-bold text-sm whitespace-nowrap">¡Cotiza aquí!</span>
          ...
          {isCartOpen && (
            <>
              ...
              <motion.div 
                ...
                animate={{ x: 0 }}
                exit={{ x: '100%' }}
                className="fixed right-0 top-0 h-full w-full max-w-md bg-background z-[70] shadow-2xl p-6 flex flex-col"
              >
                ...
                <div className="flex-1 overflow-y-auto hide-scrollbar space-y-4">
                  {cart.length === 0 ? (
                    <div className="text-center py-20">
                      <ShoppingBag size={64} className="mx-auto text-primary/10 mb-4" />
                      <p className="text-primary/40 font-medium">Tu carrito está vacío</p>
                    </div>
                  ) : (
                    cart.map(item => (
                      <div key={item.id} className="flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm">
                        ...
                        <button onClick={() => removeFromCart(item.id)} className="text-error hover:scale-110 transition-transform">
                          <X size={18} />
                        </button>
                      </div>
                    ))
                ...
                {cart.length > 0 && (
                  <div className="pt-6 border-t border-primary/5 space-y-4">
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-secondary text-white rounded-xl font-bold text-lg shadow-lg shadow-secondary/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                    >
